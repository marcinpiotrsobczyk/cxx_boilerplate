...
GITLAB_ROOT="${GITLAB_ROOT:-/boilerplate_gitlab}"
...
function clean_up {
    docker container rm -f -v cxx_boilerplate_gitlabserver_1
    docker network rm cxx_boilerplate_gitlabnetwork || true
}
trap clean_up EXIT
...
docker network create -d bridge --gateway 172.20.0.1 --subnet 172.20.0.0/16 cxx_boilerplate_gitlabnetwork

docker run --rm -dit --ip 172.20.0.2 --network cxx_boilerplate_gitlabnetwork \
    ...
    --name cxx_boilerplate_gitlabserver_1 \
    gitlab/gitlab-ce:16.7.4-ce.0


ncat --version
RETRIES=24
until ncat -z 172.20.0.2 80 || [ $RETRIES -eq 0 ]; do
    echo "waiting for gitlab ce port 80, $((RETRIES--)) remaining attempts..."
    sleep 10
done
ncat -z 172.20.0.2 80
...
docker run --rm -it --ip 172.20.0.3 --network cxx_boilerplate_gitlabnetwork \
    ...
    --docker-network-mode "cxx_boilerplate_gitlabnetwork" \
...
docker run --rm -it --ip 172.20.0.4 --network cxx_boilerplate_gitlabnetwork \
